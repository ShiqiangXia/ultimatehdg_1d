function [error_jh,error_jh_adj,estimator_ACh] = Functional_error_cal(functional_type,hs,gq_pts_phy,exact_primal_func,exact_adjoint_func,primal_num_sol,adjoint_num_sol,GQ_weights,tau_pow,grad_lag)
...
if functional_type == 1
    ...
    my_temp_AC3 = temp_AC3;
    my_temp_AC3(2,1:end-1) = (my_temp_AC3(2,1:end-1)+temp_AC3(1,2:end))*numeric_t(0.5);
    my_temp_AC3(1,2:end) = (my_temp_AC3(1,2:end)+temp_AC3(2,1:end-1))*numeric_t(0.5);
    ...
    plot(-temp1,"s-");
    %}
    title("Compare the functional error and the estimator on each element");
    legend('J(u)-J(u_h)','AC_h');
    %}
